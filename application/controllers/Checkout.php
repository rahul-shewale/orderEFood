...
class Checkout extends CI_Controller {
    ...
    public function index() {
       ...
       $user = $this->User_model->getUserById($u_id);
    ...
    public function placeOrder($u_Id) {  
        ...
        foreach($cartItems as $item) {
            ...
            $orderData[$i]['m_id'] = $item['m_id'];
            //$orderData[$i]['d_name'] = $item['name'];
            $orderData[$i]['o_quantity'] = $item['qty'];
            $orderData[$i]['o_price'] = $item['subtotal'];
            $orderData[$i]['order_date'] = date('Y-m-d H:i:s', now());
